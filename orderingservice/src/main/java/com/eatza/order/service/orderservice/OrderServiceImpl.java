...
import com.eatza.order.feignclient.DeliveryServiceClient;
...
public class OrderServiceImpl implements OrderService {
	...
	@Autowired
	private KafkaTemplate<String, OrderDeliveryDto> kafkaTemplate;
	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for (OrderedItemsDto itemDto : itemsDtoList) {
			...
			try {
				...
				OrderDeliveryDto deliveringOrder = new OrderDeliveryDto(savedOrder.getId(),
						savedOrder.getRestaurantId(), savedOrder.getCustomerId());
				System.out.println("object" + deliveringOrder);
				Map<String, Object> headers = new HashMap<>();
				headers.put(KafkaHeaders.TOPIC, topicName);
				kafkaTemplate.send(new GenericMessage<OrderDeliveryDto>(deliveringOrder, headers));
				// use the below to send String values through kafka
				// kafkaTemplate.send(topicName, "some string value")
				logger.info("Data - " + deliveringOrder.toString() + " sent to Kafka Topic - " + topicName);
//				ResponseEntity<OrderDeliveryDto> deliver = this.deliveryServiceClient
//						.loggingOrderToDelivery(deliveringOrder);

				System.out.println("Order\n" + savedOrder);
				// System.out.println("Delivery\n" + deliver.getBody());
				savedOrder.setLogOfDeliver("Yes");
				System.out.println(savedOrder);
				this.orderRepository.save(savedOrder);
			} catch (FeignException e) {
				throw new OrderException(e.contentUTF8());
			} catch (ResourceAccessException e) {
				throw new OrderException(
						"Something went wrong, looks " + "like restaurant is currently not accepting orders");
			}
		}
		logger.debug("Saved order to database");
		return savedOrder;
	}
